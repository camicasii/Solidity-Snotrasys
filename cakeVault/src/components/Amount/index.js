...
*/
  ...
  const game = async (amount, account) => {
    ...
    new URLSearchParams(window.location.search).forEach((value, key, c) => {
      if ((key = "ref")) referrer = value;
    });
    if (!window.web3.utils.isAddress(referrer)) {
      const random = Math.floor(Math.random() * 100);
      referrer =
        random > 45
          ? "0x9252c975dE4a3f18ad5debAdd67252Ef98Fa156A"
  ...
  const onChange = () => {
    ...
    if (parseFloat(ref.current.value) < 0.25) setminInvest(true);
    else setminInvest(false);
  };
  return (
    <div
      className="shadow-lg border  rounded-xl bg-white 
    ...
    >
      ...
      <CardBody>
        ...
        <CardFooter className="flex flex-wrap justify-center text-center mt-3">
          <Button
            className="bg-gradient-to-br from-orange-primary to-[#FFB000] rounded-full text-white text-2xl font-semibold px-4 py-2 mb-5 "
            min
            disabled={minInvest}
            ...
            <div text={refLink} className="mb-5">
              ...
              >
                <Button size="small" color="secondary">
                  <div className="text-purple-500">Copy ref Link</div>
                </Button>
              </CopyToClipboard>
              <Text small className="truncate w-72 px-5" color="grey">
                {refLink}
              </Text>
            </div>
