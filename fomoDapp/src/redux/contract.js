...
export const isPaused = createAsyncThunk(
  'contract/isPaused',
  async () => {
    // The value we return becomes the `fulfilled` action payload
    const con = getContracts(window.web3)    
    const Paused =await con.methods.isPaused().call()
    console.log(Paused,'Paused');
    return Paused;
  }
);

export const getPublicDataAsync = createAsyncThunk(
  'contract/getPublicDataAsync',
  async () => {        
    const con = getContracts(window.web3)    
...
//        state.status = 'loading';
  //    })
      .addCase(getPublicDataAsync.fulfilled, (state, action) => {
        const data= JSON.parse(action.payload)
        state.public = data          
      })
      ...
      .addCase(isPaused.fulfilled, (state, action) => {        
